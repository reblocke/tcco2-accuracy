...
SD 3.51 // variance in the intra-study agreement between measurements 
LOAL = -7.1
LOAU = 6.9*/
set seed 12345
...
gen error_all = rnormal(0, sqrt(`corr_sd'^2 + `tau'^2)) - `mean_bias' 
summ error_all, detail //check
macro drop _all
...
LOAU = 3.9*/
set seed 12345
local mean_bias = -0.1 //mean bias from raw data
local tau = 1.4 //variance in the between-study estimates of mean bias
local corr_sd 1.6 // variance in the intra-study agreement between measurements
gen error_amb = (rnormal(0, sqrt(`corr_sd'^2 + `tau'^2)) - `mean_bias') if is_amb==1 & is_cc!=1
...
LOAU = 7.1*/
set seed 12345
local mean_bias = 1.7 //mean bias from raw data
local tau = 3.2 //variance in the between-study estimates of mean bias
local corr_sd 2.0 // variance in the intra-study agreement between measurements
gen error_inp_emerg = (rnormal(0, sqrt(`corr_sd'^2 + `tau'^2)) - `mean_bias') if ((is_cc~=1) & (is_inp==1 | is_emerg==1))
summ error_inp, detail //check
macro drop _all
...
local tau_cc = 1.9 //variance in the between-study estimates of mean bias
...
*Inpatient+Emergency:
gen tcco2_inp_emerg = paco2 + error_inp_emerg if  & ((is_inp==1 & is_cc!=1) | is_emerg ==1)) & !missing(paco2) 
*ICU: 
...
program define simulate_tcco2, rclass
    ...
    preserve
        ...
        gen tcco2_mc = paco2 + bias_mc + rnormal(0, sd_mc)
...
summarize mean_tcco2_mc sd_tcco2_mc
restore
