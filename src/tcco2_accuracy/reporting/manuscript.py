...
def run_manuscript_outputs(
    ...
    policy = TwoStagePolicy(
        lower=two_stage_lower, upper=two_stage_upper, true_threshold=true_threshold
    )
...
def _paco2_stats(prepared: pd.DataFrame, true_threshold: float) -> pd.DataFrame:
    patient_col = next(
        (
            col
            for col in ("patient_id", "subject_id", "patient", "subject")
...
def _build_table1(
    sim_summary: pd.DataFrame, paco2_stats: pd.DataFrame, true_threshold: float
) -> pd.DataFrame:
...
def _order_groups(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ordered["group"] = pd.Categorical(
        ordered["group"], categories=MANUSCRIPT_GROUP_ORDER, ordered=True
    )
    sort_columns = ["group"]
