...
- Invariants: `tests/test_conway_meta.py` validates bias/SD/τ²/LoA against Table 1.

## Workflow validation stages

### Meta-analysis reproduction
- Purpose: recompute bias/SD/τ²/LoA per subgroup from Conway inputs.
- Invariants: LoA identity checks in `src/tcco2_accuracy/workflows/meta.py` and tests in `tests/test_conway_meta.py`.
...
- Invariants: reproducibility and τ² >= 0 checks in `tests/test_bootstrap.py`.
...
- Invariants: subgroup assignment and quantile checks in `tests/test_paco2_distribution.py`.
...
- Invariants: moment/interval checks in `tests/test_simulation.py`.
...
- Invariants: likelihood/prior checks in `tests/test_inference.py` and determinism in `tests/test_workflows.py`.
...
- Invariants: probability mass and branching checks in `tests/test_hybrid_bootstrap_and_conditional.py`.
...
- Invariants: smoke test in `tests/test_manuscript_workflow.py`.
...
- Invariants: `tests/test_browser_contract.py` compares contract outputs to `predict_paco2_from_tcco2`
...
- Staging: `tests/test_stage_web_python.py` verifies package/data staging into `web/assets/`.
