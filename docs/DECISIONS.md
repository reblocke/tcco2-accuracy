...
- Conway study exports read bias/S2 and subgroup membership from the RData objects (`main`, `ICU`, `ARF`, `LFT`) and merge counts from `data.dta`; the Bolliger ICU row uses a 49/49/1 count fallback with bias/S2 pulled from `ICU` when absent from `main` to preserve Table 1 reproduction (`scripts/export_conway_rdata.py:29`, `scripts/export_conway_rdata.py:96`).
