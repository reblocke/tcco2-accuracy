...
- Main-analysis descriptive counts aggregate by `study_base` (strip trailing parentheses) and treat identical-bias multi-row citations as overlapping cohorts (use max counts) in `python/src/tcco2_accuracy/conway_meta.py:221`.
...
- PaCO2 subgroups map to Conway bootstrap groups via the shared helper in `python/src/tcco2_accuracy/_params.py` (pft→lft, ed_inp→arf, icu→icu, all→main) to align ambulatory and acute respiratory failure sub-analyses.
...
- When subgroup-specific parameters are missing, simulation and inference use the shared selector in `python/src/tcco2_accuracy/_params.py` and fall back to all parameters (with a warning) rather than dropping the subgroup.
