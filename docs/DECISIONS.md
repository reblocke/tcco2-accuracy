...
- Bootstrap τ² draws are truncated at 0 to enforce non-negative between-study variance for simulation/inference draws in `src/tcco2_accuracy/bootstrap.py`.
- Bootstrap workflows default to `cluster_plus_withinstudy` to align outer CI scale with Conway, while low-level bootstrap functions default to `cluster_only` in `src/tcco2_accuracy/workflows/bootstrap.py` and `src/tcco2_accuracy/bootstrap.py`.
- Hybrid bootstrap perturbations treat bias and log-variance inputs as independent due to missing covariance data in `src/tcco2_accuracy/bootstrap.py`.
- Meta-analysis τ² defaults to untruncated values to reproduce Conway Table 1, but if the method-of-moments denominator is non-finite or zero we set τ² = 0 for stability in `src/tcco2_accuracy/conway_meta.py`.
- LoA confidence intervals are undefined for single-study summaries, so CI bounds are returned as NaN when df ≤ 0 in `src/tcco2_accuracy/conway_meta.py`.
- Main-analysis descriptive counts aggregate by `study_base` (strip trailing parentheses) and treat identical-bias multi-row citations as overlapping cohorts (use max counts) in `src/tcco2_accuracy/conway_meta.py`.
...
- PaCO2 subgroups map to Conway bootstrap groups via the shared helper in `src/tcco2_accuracy/_params.py` (pft→lft, ed_inp→arf, icu→icu, all→main) to align ambulatory and acute respiratory failure sub-analyses.
- Simulation/inference parameter validation requires finite numeric values with non-negative σ² and τ² in `src/tcco2_accuracy/utils.py`.
- When subgroup-specific parameters are missing, simulation and inference use the shared selector in `src/tcco2_accuracy/_params.py` and fall back to all parameters (with a warning) rather than dropping the subgroup.
